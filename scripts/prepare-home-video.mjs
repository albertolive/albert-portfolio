...
Usage: node scripts/prepare-home-video.mjs --input VIDEO --out DIRECTORY
Example: node scripts/prepare-home-video.mjs --input ~/Downloads/hero.mp4 --out /tmp/portfolio-hls
...
`;
const { values } = parseArgs({ options: { input: { type: "string" }, out: { type: "string" }, help: { type: "boolean" } } });
...
const settings = {
  ...
  segmentSeconds: 4,
  renditions: [
    { name: "1080", width: 1920, height: 1080, crf: 19, maxrate: 5000 },
    { name: "720", width: 1280, height: 720, crf: 23, maxrate: 2200 },
...
try {
  ...
  try {
    ...
    console.log(JSON.stringify({ release, directory: destination, variants: existing.variants, ...urls, reused: true }, null, 2));
    process.exit(0);
  } catch (error) {
  ...
  for (const rendition of settings.renditions) {
    ...
    run("ffmpeg", ["-hide_banner", "-loglevel", "error", "-nostdin", "-i", input,
  ...
  run("ffmpeg", ["-hide_banner", "-loglevel", "error", "-nostdin", "-i", input, "-map", "0:v:0", "-frames:v", "1", "-vf", "scale=w='min(1920,iw)':h='min(1080,ih)':force_original_aspect_ratio=decrease", "-q:v", "2", join(staging, "poster.jpg")]);
  ...
  await writeFile(join(staging, "release.json"), JSON.stringify({ release, variants, files: checksums }, null, 2) + "\n");
  await rename(staging, destination);
  staging = undefined;
  console.log(JSON.stringify({ release, directory: destination, variants, ...urls, reused: false }, null, 2));
} catch (error) {
