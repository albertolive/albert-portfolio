...
try {
  ...
  for (const [width, height] of [[1440,900],[1280,800],[768,1024],[390,844],[320,740]]) {
    ...
    await page.getByRole("button", { name: /^AI product engineer$/i }).click();
    assert.equal(await page.locator('p[aria-live="polite"]').textContent(), "R 0 / 10");
    await page.getByRole("button", { name: /^AI product engineer$/i }).click();
    await page.waitForTimeout(4500);
    const result = await page.evaluate(() => {
      const prose = document.querySelector("main > div");
      ...
      return { filter: getComputedStyle(prose).filter, opacity: getComputedStyle(prose).opacity,
        ...
        animations: prose.getAnimations({ subtree: true }).length,
        overflow: document.documentElement.scrollWidth > innerWidth };
    });
    assert.deepEqual(result, { filter: "none", opacity: "1", openFilter: "none", openOpacity: "1", animations: 0, overflow: false });
  ...
  console.log("PASS metadata, five viewports, clear text after 4.5s, nested counters, motion changes, blocked/failed/slow/no-JS poster");
} finally {
  await browser.close();
}
